...
try:
    ...
    # from PyQt5.QtWebEngineWidgetsimport QWebView
...
class mainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.actionMV_oberrhein.triggered.connect(
            partial(self.load_pandapower_network, pnw.mv_oberrhein, "MV Oberrhein")
        )
        ...
        self.actionattack.triggered.connect(self.attack)
        # self.actionattack.triggered.connect(self.lossesSummary)
        self.actionattack.setIcon(QIcon("resources/icons/attack.svg"))
    ...
    def attack(self):
        num_times = 24
        p_load_data = np.random.randn(self.net.load.shape[0], num_times)
        q_load_data = np.random.randn(self.net.load.shape[0], num_times)
        self.update_loads(p_load_data[:, 0], q_load_data[:, 0])
        pp.runpp(
            self.net,
            enforce_q_lims=True,
    ...
    def runpp(self):
        pf_res_plotly(
            self.net,
            filename="/home/mdeogune/Downloads/pandapower-gui-master/temp-plot.html",
        )
        try:
    ...
    def lossesSummary(self):
            ...
            if "res" in i:
                if "pl_kw" in self.net[i]:
                    if not self.net[i]["pl_kw"].empty:
                        ...
                        self.main_message.append(i)
                        self.main_message.append(self.net[i]["pl_kw"].to_string())
        ...
        # get total load
        total_load_kw = (
            self.net.res_gen.sum()
            + self.net.res_sgen.sum()
            + self.net.res_ext_grid.sum()
        )
        self.main_message.append("Total nett load flowing in network")
        self.main_message.append(str(total_load_kw["p_mw"]))

        # losses percentage
        self.main_message.append("% losses")
        loss_pct = losses / total_load_kw["p_mw"]
    ...
    def drawCollections(self):
        ...
        # self.canvas.load(QUrl("https://pandapower.readthedocs.io"))
        self.canvas.load(
            QUrl().fromLocalFile(
                "/home/mdeogune/Downloads/pandapower-gui-master/temp-plot.html"
            )
        )
        # self.canvas.setHtml(
        #     "/home/mdeogune/Downloads/pandapower-gui-master/temp-plot.html"
        # )
        # # self.canvas.setWindowTitle("pandapower Documentation")
        # self.canvas.show()
    ...
    def updateLoadCollections(self, redraw=False):
        l1, l2 = plot.create_load_collection(
            self.net,
            size=self.scale * 0.02,
            picker=True,
    ...
    def updateGenCollections(self, redraw=False):
        l1, l2 = plot.create_gen_collection(
            self.net, size=self.scale * 0.02, picker=True, infofunc=lambda x: ("gen", x)
        )
    ...
    def onCollectionsClick(self, event):
        ...
        if self.create_bus.isChecked():
            ...
            try:
                self.bus_window = BusWindow(
                    self.net, self.updateBusCollection, geodata=geodata
                )
    ...
    def open_element_window(self, element, index):
        if element == "bus":
            ...
            self.element_window = BusWindow(
                self.net, self.updateBusCollection, index=index
            )
        elif element == "line":
            print("will bild line")
            print(index)
            self.element_window = LineWindow(
                self.net, self.updateLineCollection, index=index
            )
        elif element == "load":
            self.element_window = LoadWindow(
                self.net, self.updateLoadCollections, index=index
            )
        elif element == "gen":
            self.element_window = GenWindow(
                self.net, self.updateGenCollections, index=index
            )
        elif element == "ext_grid":
            self.element_window = ExtGridWindow(
                self.net, self.updateExtGridCollections, index=index
            )
        elif element == "trafo":
        ...
        if self.create_line.isChecked():
            ...
            if self.lastBusSelected is None:
                ...
                pp.create_transformer(
                    self.net, self.lastBusSelected, index, std_type="0.25 MVA 10/0.4 kV"
                )
                self.lastBusSelected = None
                self.updateTrafoCollections()
                self.drawCollections()
            ...
            try:
                self.load_window = LoadWindow(
                    self.net, self.updateLoadCollections, bus=index
                )
            except Exception as e:
                print(e)
            self.lastBusSelected = None
            ...
            try:
                self.gen_window = GenWindow(
                    self.net, self.updateGenCollections, bus=index
                )
            except Exception as e:
                print(e)
            self.lastBusSelected = None
            ...
            try:
                self.ext_grid_window = ExtGridWindow(
                    self.net, self.updateExtGridCollections, bus=index
                )
            except Exception as e:
                print(e)
            self.lastBusSelected = None
...
class runppOptions(QDialog):
    ...
    def get_parameters(self):
        ...
        self.net._runpp_options[
            "voltage_dependent_loads"
        ] = self.VoltageDependent.isChecked()
...
def createSampleNetwork():
    net = pn.case_ieee30()
    # net = pp.create_empty_network()
    # b1 = pp.create_bus(net, vn_kv=20.0, name="HV", geodata=(5, 30))
